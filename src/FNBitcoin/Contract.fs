...
module Contract =
    ...
    let TimeOut (pubkey : PubKey, time : DateTimeOffset) (amount : Money) =
        let scriptStr =
            sprintf "%d OP_CHECKSEQUENCEVERIFY OP_DROP %s OP_DROP" 
                (time.ToUnixTimeSeconds()) (pubkey.ToHex())
        Verified(Script(scriptStr))
    
    let EscrowWithDelay (sender : PubKey, recipient : PubKey, escrow : PubKey, 
                         delay : DateTimeOffset) (amount : Money) =
        verify { let! multiSigScript = Multisig
                                           (2, [| sender; recipient; escrow |])
                 let! timeoutScript = TimeOut (sender, delay) (amount)
                 return [| multiSigScript, timeoutScript |] }
    
    let SpendEscrowWithDelay(source : Transaction, scripts : Script array, 
                             output : BitcoinAddress, amount : Money) =
        let txb = source.GetConsensusFactory().CreateTransactionBuilder()
        txb.AddKnownRedeems(scripts) |> ignore
        txb.AddCoins(source.Outputs.AsCoins() |> Seq.map (fun c -> c :> ICoin)) 
        |> ignore
